...
//21.1 : Update - If line ends, mode is set to manual 
//21.2 : Update - If both IR Sensors detect nothing, the bot will stop automatically

#include <SoftwareSerial.h>
SoftwareSerial BT_Serial(2, 3); // arduino RX, TX; bt TX RX
...
if((digitalRead(R_S) == 1)&&(digitalRead(L_S) == 1)){Stop();mode=0;}     //if Right Sensor and Left Sensor are at Black color then it will call Stop function
...
//===============================================================================     
 ...
 Serial.print("S=");Serial.println(distance_F);
  if (distance_F > set){forward();}
    ...
    if(bt_ir_data == 16){mode=0; Stop();delay(2000);}
    if((digitalRead(R_S) == 1)&&(digitalRead(L_S) == 1)){backward();delay(300);mode=0;} 
}
